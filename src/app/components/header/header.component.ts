          ...
          <div class="hidden md:flex items-center gap-4 flex-wrap justify-end">
            @if (authReady() ? auth.isAuthenticated() : auth.authHint()) {
              ...
              <a routerLink="/favorites" class="hover:text-mb-cyan transition" aria-label="Favorites" title="Favorites">
                <span class="text-2xl inline-block select-none" style="color: #FFD700; background-color: #BDBDBD; font-family: system-ui, sans-serif; line-height: 32px; height: 32px;">★</span>
              </a>
