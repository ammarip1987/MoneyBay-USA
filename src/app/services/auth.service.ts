...
export class AuthService {
   ...
   * обновление страницы выбрасывало вошедшего, пока подъём по куке не
   * подхватился.
   *
  ...
  /**
   * Токен доступа держится в памяти вкладки и дублируется в хранилище:
   * после перезагрузки память пуста, и без копии шапка показывала бы гостя
   * до ответа сервера. Подъём идёт по куке, до которой скрипты не дотягиваются,
   * либо по обновляющему токену из хранилища, если куку стёрли.
   */
   ...
   *
   * Пробуется всегда, а не только при наличии метки: метка ставится скриптом и
   * пропадает от чистки данных сайта, тогда как кука жива — и вход, который
   * можно было поднять, не поднимался.
   *
   * Вход гасится только при явном отказе сервера. Сетевая неудача его не
   * трогает: иначе один сбой связи при загрузке страницы выкидывал бы
  ...
  refreshToken(): Observable<LoginResponse> {
    ...
    this.refreshing = this.http
      .post<LoginResponse>(`${this.baseUrl}/api/auth/refresh`, {}, {
        ...
        // Токен идёт и заголовком: кука работает, но пропадает от чистки
        // данных сайта, а заголовок берётся из хранилища и переживает её
  ...
  setSession(res: LoginResponse): void {
    // Токен кладётся и в хранилище: после перезагрузки память пуста, и до
    // ответа сервера шапка иначе показывала бы гостя
