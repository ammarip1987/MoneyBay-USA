...
export class HomeComponent implements OnInit, OnDestroy {
  ...
  loadListings(append = false): void {
    ...
    // Прежние карточки убираются до запроса, даже когда ответ уже в кэше:
    // иначе при переходе в раздел на экране оставались объявления с главной,
    // а поверх них ложился скелет — виднелось и то, и другое разом
    if (!append && !this.pagingNow()) {
      this.listings.set([]);
    }

    if (!append && !this.api.hasCachedListings(params)) {
      this.loading.set(true);
      // Очистка уже сделана выше, здесь только пометка загрузки
    }
