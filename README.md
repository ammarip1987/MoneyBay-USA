...
запуске, а `HttpsConnectorConfig` поднимает на этом сертификате второй порт
8443. Правило Origin Rules в Cloudflare переводит обращения с 443 на 8443,
...
Возврат: создать Application Load Balancer с целевой группой на порт 8080,
подключить его к службе ECS, вернуть записи `api.moneybay.us` тип CNAME со
значением адреса балансировщика, убрать правило Origin Rules и шаг обновления
DNS из прогона. Описание задачи и группа безопасности править не нужно —
порт 8080 там остался.
